...
export async function publicRoute(request,env,url){
  ...
  if(url.pathname==='/api/public/home'){
    const [featured,recent,fields,units]=await Promise.all([
      env.DB.prepare(`SELECT d.*,u.name unit_name,u.short_name unit_short_name,f.name field_name,c.name category_name FROM documents d LEFT JOIN units u ON u.id=d.unit_id LEFT JOIN fields f ON f.id=d.field_id LEFT JOIN categories c ON c.id=d.category_id WHERE d.status='published' AND d.featured=1 ORDER BY d.published_at DESC,d.updated_at DESC,d.created_at DESC LIMIT 6`).all(),
      env.DB.prepare(`SELECT d.*,u.name unit_name,u.short_name unit_short_name,f.name field_name,c.name category_name FROM documents d LEFT JOIN units u ON u.id=d.unit_id LEFT JOIN fields f ON f.id=d.field_id LEFT JOIN categories c ON c.id=d.category_id WHERE d.status='published' ORDER BY COALESCE(d.updated_at,d.published_at,d.created_at) DESC,d.published_at DESC,d.created_at DESC LIMIT 6`).all(),
      env.DB.prepare("SELECT * FROM fields WHERE status='active' ORDER BY sort_order,name LIMIT 12").all(),
      env.DB.prepare(`SELECT u.*,COUNT(d.id) document_count FROM units u LEFT JOIN documents d ON d.unit_id=u.id AND d.status='published' WHERE u.status='active' GROUP BY u.id ORDER BY u.sort_order,u.name LIMIT 8`).all()
    ]);
    return json({
      featured:(featured.results||[]).map(normalizeDoc),
      recent:(recent.results||[]).map(normalizeDoc),
