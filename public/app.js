...
async function home(){
  ...
  setMain(`
    ...
    <section class="intent-section" aria-labelledby="intentTitle">
      <div class="container">
        <div class="intent-head">
          <div>
            <span class="eyebrow">Bắt đầu nhanh</span>
            <h2 id="intentTitle">Bạn đang tìm gì?</h2>
          </div>
          <p>Đi thẳng tới đúng loại học liệu thay vì phải dò từng menu.</p>
        </div>

        <div class="intent-grid">
          <a class="intent-card" href="/materials/textbooks">
            <span class="intent-icon">01</span>
            <div><b>Giáo trình</b><small>Tài liệu học theo chương trình và lộ trình.</small></div>
          </a>
          <a class="intent-card" href="/materials/lessons">
            <span class="intent-icon">02</span>
            <div><b>Bài giảng</b><small>Nội dung hỗ trợ quá trình dạy và học.</small></div>
          </a>
          <a class="intent-card" href="/materials/exercises">
            <span class="intent-icon">03</span>
            <div><b>Bài tập</b><small>Phiếu luyện tập và nội dung thực hành.</small></div>
          </a>
          <a class="intent-card" href="/materials/exams">
            <span class="intent-icon">04</span>
            <div><b>Ngân hàng đề</b><small>Đề luyện tập và tài liệu đánh giá.</small></div>
          </a>
          <a class="intent-card" href="/academic/research">
            <span class="intent-icon">05</span>
            <div><b>Nghiên cứu</b><small>Nội dung nghiên cứu và sản phẩm học thuật.</small></div>
          </a>
        </div>
      </div>
    </section>
    ...
    <section class="section" id="featuredSection" hidden>
      <div class="container">
        <div class="section-head">
          <div>
            <span class="section-kicker">Được chọn lọc</span>
            <h2>Học liệu nổi bật</h2>
            <p>Những tài liệu đang được SFN hoặc các đơn vị học thuật ưu tiên giới thiệu.</p>
          </div>
          <a class="btn btn-secondary" href="/library">Xem toàn bộ</a>
        </div>

        <div class="cards" id="homeFeatured"></div>
      </div>
    </section>
    ...
    <section class="section soft">
      <div class="container">
        <div class="section-head">
          <div>
            <span class="section-kicker">Đang hoạt động</span>
            <h2>Mới cập nhật</h2>
            <p>Các tài liệu được công bố hoặc cập nhật gần đây trên hệ thống.</p>
          </div>
          <a class="btn btn-ghost" href="/library">Xem thư viện</a>
        </div>

        <div class="cards" id="homeRecent">${empty('Đang tải tài liệu mới...')}</div>
      </div>
    </section>
    ...
    <section class="section">
      <div class="container">
        <div class="section-head">
          <div>
            <span class="section-kicker">Hệ sinh thái học thuật</span>
            <h2>Các đơn vị trên Academic Hub</h2>
            <p>Mỗi đơn vị có hồ sơ, lĩnh vực và học liệu riêng trên cùng một hệ thống.</p>
          </div>
          <a class="btn btn-secondary" href="/units">Xem tất cả đơn vị</a>
        </div>

        <div class="home-unit-grid" id="homeUnits">${empty('Đang tải danh sách đơn vị...')}</div>
      </div>
    </section>
    ...
    <section class="section soft">
      <div class="container">
        <div class="section-head">
          <div>
            <span class="section-kicker">Khám phá</span>
            <h2>Theo lĩnh vực</h2>
  ...
  try{
    ...
    if(h.featured.length){
      ...
      $('#homeFeatured').innerHTML=h.featured.map(docCard).join('');
    }

    $('#homeRecent').innerHTML=
      h.recent.length
        ? h.recent.map(docCard).join('')
        : empty('Chưa có tài liệu công khai để hiển thị.');

    $('#homeUnits').innerHTML=
      h.units.length
        ? h.units.map(
            u=>`
              <a class="home-unit-card" href="/unit/${encodeURIComponent(u.slug)}">
                <div class="home-unit-mark">${esc(u.short_name||u.code||'SFN')}</div>
                <div class="home-unit-body">
                  <span class="home-unit-type">${esc(u.unit_type||'Đơn vị học thuật')}</span>
                  <h3>${esc(u.name)}</h3>
                  <p>${esc(u.description||'Đơn vị tham gia hệ sinh thái học thuật Sky First.')}</p>
                  <div class="home-unit-meta">
                    <span>${esc(u.code)}</span>
                    <span>${fmt(u.document_count)} tài liệu</span>
                  </div>
                </div>
              </a>
            `
          ).join('')
        : empty('Chưa có đơn vị được công bố.');
    ...
    $('#homeRecent').innerHTML=empty('Không thể tải dữ liệu lúc này.');
    $('#homeUnits').innerHTML=empty('Không thể tải danh sách đơn vị lúc này.');
      ...
      <div class="container document-layout">
        ...
        <aside class="document-side">
          ...
          <div class="document-feedback">
            <b>Phát hiện thông tin chưa chính xác?</b>
            <p class="small">Gửi góp ý về metadata, liên kết lỗi hoặc vấn đề bản quyền và kèm mã tài liệu <strong>${esc(x.code)}</strong>.</p>
            <a class="btn btn-ghost" href="/contact">Báo lỗi / góp ý tài liệu</a>
          </div>
